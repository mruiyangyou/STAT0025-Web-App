...
class Input(object):
    ...
    def __init__(self, cost_coef, resources_coef, resources, product_name,
                    resource_name, save_name, num_of_products = 2) -> None:
...
class SolveGraphicSolution(object):
    ...
    def plot(self, input, x_max, y_max, res, doc, x, value):
        plt.figure(dpi = 300)
        ...
        plt.savefig(os.path.join('src/Linear-Progamming-Visulization/GraphicSolution/saveimgs',
                            f'{input.save_name}'))

    def to_frame(self,input, x, value):
